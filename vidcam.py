...
@author: Tommaso
"""
import cv2, time, multiprocessing,  PIL.Image, pyglet
...
class WEBCAM():
    ...
    def capture(self):
        ...
        while self.activation == True:
            ...
            self.OUT = cv2.resize(cv2.flip(frame,1),(self.width, self.height),interpolation=cv2.INTER_NEAREST)   # saving frame
        return()        



class Webcam_feedback(pyglet.window.Window):
    """Function to see a feeback from the webcam
...
if __name__ == "__main__":
    ...
    # Saving videos in parallel process
    process2.start()
    time.sleep(8)     # wait for everything to start up
    activator_manager.value=2   # starting to save the frames
    time.sleep(15)      # HERE YOU CAN ADD WHATEVER YOU WANT (e.g. videos or images)
    activator_manager.value=1   # stop the webcam
    process2.join()
